...
for collection in $COLLECTIONS; do
  status=$(curl -s -o /dev/null -w "%{http_code}" -H "api-key: ${QDRANT_API_KEY}" "${QDRANT_URL}/collections/${collection}")
  if [ "$status" = "200" ]; then
    schema=$(curl -sf -H "api-key: ${QDRANT_API_KEY}" "${QDRANT_URL}/collections/${collection}")
